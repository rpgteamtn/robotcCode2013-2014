...
//  waitForStart(); // Wait for the beginning of autonomous phase.
  ...
  if(SensorValue(HTSMUX) == 6)
  {
  turnDeg(45, TURN_RIGHT, waitTime, 50);
  moveDist(53.34, waitTime, 50);
 	// put block in basket
  moveDist(-17.78, waitTime, 50);
  turnDeg(135, TURN_RIGHT, waitTime, 50);
  moveDist(144.78, waitTime, 50);
  turnDeg(90, TURN_LEFT, waitTime, 50);
  moveDist(93.98, waitTime, 50);
  turnDeg(90, TURN_LEFT, waitTime, 50);
  moveDist(93.98, waitTime, 50);
  stops();
	}

	if(SensorValue(HTSMUX) == 4)
	{

	turnDeg(45, TURN_LEFT, waitTime, 50);
  moveDist(53.34, waitTime, 50);
 	// put block in basket
  moveDist(-17.78, waitTime, 50);
  turnDeg(135, TURN_LEFT, waitTime, 50);
  moveDist(144.78, waitTime, 50);
  turnDeg(90, TURN_RIGHT, waitTime, 50);
  moveDist(93.98, waitTime, 50);
  turnDeg(90, TURN_RIGHT, waitTime, 50);
  moveDist(93.98, waitTime, 50);
  stops();
